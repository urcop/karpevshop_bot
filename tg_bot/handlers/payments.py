...
from tg_bot.keyboards import inline_payment, reply_main_menu
from tg_bot.keyboards import reply_back_to_main
from tg_bot.keyboards.inline_payment import payment_choice_callback
...
async def payment(message: types.Message):
    await message.answer('💳 Введите сумму в рублях', reply_markup=reply_back_to_main.keyboard)
        ...
        try:
            ...
            if data['amount'] >= min_payment:
                await message.answer(f'Вы хотитите пополнить баланс на {data["amount"]} руб.\n'
                                     f'Выберите способ оплаты:', reply_markup=inline_payment.choice_keyboard)
            else:
    ...
    async with state.proxy() as data:
        ...
        if payment_system == 'QIWI':
            ...
            await call.message.edit_text(
                ...
                reply_markup=inline_payment.generate_qiwi_keyboard(url.invoice)
            )
            await state.finish()
            ...
            await call.message.answer(
                ...
                reply_markup=inline_payment.check
            )
            await state.finish()
...
async def payment_success(call: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await call.message.answer('Успешно оплачено', reply_markup=reply_main_menu.keyboard)
    await state.finish()
...
async def get_payment_check(message: types.Message, state: FSMContext):
    if message.content_type in ['photo', 'document']:
        ...
        await message.answer('Чек успешно отправлен', reply_markup=reply_main_menu.keyboard)
        await state.finish()
    else:
...
def register_payments(dp: Dispatcher):
    dp.register_message_handler(payment, text="Пополнить баланс 💳")
    dp.register_message_handler(get_amount, state=PaymentState.amount)
    dp.register_callback_query_handler(get_payment, payment_choice_callback.filter(), state=PaymentState.amount)
